...
window.config(bg="#25425c")
game = ttt()

# Welcome
welframe = Frame(window)
welframe.pack()
welframe.config(bg="#2c4f6e")
wel = "Tic-Tac-Toe with Minimax AI"
welcome = Label(welframe, text="╔"+"═"*(len(wel)-10)+"╗\n║ "+wel+" ║\n╚"+"═"*(len(wel)-10)+"╝", 
                font=('OCR A Extended', 20, ), bg="#2c4f6e", fg="white", padx=10)
welcome.pack(pady=10)

# Scoreboard
winframe = Frame(window)
winframe.pack()
winframe.config(bg="#2c4f6e")
scores = StringVar()
...
infoframe.pack()
infoframe.config(bg="#2c4f6e")
info = StringVar()
info.set("Player X starts the game!")
infoboard = Label(infoframe, textvariable=info, font=('OCR A Extended', 15), bg="#2c4f6e", fg="white")
...
tttgrid.pack()
for b in range(9):
    block = Button(tttgrid, width=5, height=2, font=('terminus', 30), disabledforeground="black",
...
resetframe.pack()
resetframe.config(bg="#2c4f6e")
resetbutton = Button(resetframe, text="Replay", font=('OCR A ESxtended', 15), command=game.reset)
